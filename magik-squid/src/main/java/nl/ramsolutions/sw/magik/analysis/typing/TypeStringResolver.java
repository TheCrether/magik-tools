...
public class TypeStringResolver {
	...
	private final Map<Map.Entry<TypeString, String>, Collection<MethodDefinition>> methodsCache = new HashMap<>();
		...
		return this.getPackageHierarchy(typeString).stream()
				...
				.flatMap(
						def -> {
							...
							final TypeString pkgTypeString = TypeString.ofIdentifier(typeString.getIdentifier(),
									packageName);
	...
	/**
	 * Get the {@link ITypeStringDefinition} for the given {@link TypeString},
	 * following package uses.
	 *
	 * @param typeString Reference to look for.
	 * @return A
	 *         {@link ExemplarDefinition}/{@link ProcedureDefinition}/{@link GlobalDefinition}.
	 */
	public synchronized Collection<ITypeStringDefinition> resolve(final TypeString typeString) {
		return this.typeCache.computeIfAbsent(
				typeString,
				typeStr -> {
					final Collection<ExemplarDefinition> exemplarDefinitions = this.findExemplarDefinitions(typeStr);
					final Collection<ProcedureDefinition> procedureDefinitions = this.findProcedureDefinitions(typeStr);
					final Collection<GlobalDefinition> globalDefinitions = this.findGlobalDefinitions(typeStr);
					return Stream.of(
	 ...
	 *
	 * <p>
	 * Note that this gives only a singular {@link ExemplarDefinition}, even though
	 * there might be
	 * multiple known.
	 *
	...
	public ExemplarDefinition getExemplarDefinition(final TypeString typeString) {
		...
		final ITypeStringDefinition exemplarDefefinition = definitions.stream()
				.filter(ExemplarDefinition.class::isInstance).findAny().orElse(null);
		final ITypeStringDefinition definition = exemplarDefefinition != null ? exemplarDefefinition
	...
	public boolean isKindOf(final TypeString typeString1, final TypeString typeString2) {
		for (final TypeString typeStr1 : TypeString.combine(typeString1).getCombinedTypes()) {
			final ITypeStringDefinition definition1 = this.resolve(typeStr1).stream().findAny().orElse(null);
			if (definition1 == null) {
				continue;
			}
			...
			for (final TypeString typeStr2 : combinedTypeString2.getCombinedTypes()) {
				final ITypeStringDefinition definition2 = this.resolve(typeStr2).stream().findAny().orElse(null);
				if (definition2 == null) {
					continue;
				}
	...
	/**
	 * Get the {@link MethodDefinition}s the {@link TypeString} responds to,
	 * including from its super
	 * types.
	 *
	 * @param typeString {@link TypeString} to resolve.
	...
	public synchronized Collection<MethodDefinition> getMethodDefinitions(
		...
		return this.methodsCache.computeIfAbsent(
				...
				entry -> {
					...
					final TypeString actualTypeStr = resolvedTypes.isEmpty()
							? typeString
	...
	public synchronized Collection<MethodDefinition> getMethodDefinitions(
		...
		return this.methodsCache.computeIfAbsent(
				cacheKey,
				entry -> methodDefinitions.stream()
						...
						typeStr -> this.definitionKeeper
								...
								.forEach(
										methodDefinition -> {
											...
											final Set<MethodDefinition> methodsForName = methodDefinitions
													.computeIfAbsent(methodName, key -> new HashSet<>());
	...
	private Collection<TypeString> getParents(final ITypeStringDefinition definition) {
		if (definition instanceof ExemplarDefinition exemplarDefinition) {
			...
			final ITypeStringDefinition aliasedDefinition = this.resolve(typeString).stream().findAny().orElse(null);
	 ...
	 *
	 * <p>
	 * This add the implicit parents, where {@link ExemplarDefinition} only returns
	 * its explicitly
	 * defined parents.
	 *
	 * @param typeString
