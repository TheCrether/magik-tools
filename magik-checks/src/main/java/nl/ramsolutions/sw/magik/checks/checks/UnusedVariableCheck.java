...
public class UnusedVariableCheck extends MagikCheck {
  ...
  private static final String PARAM_MESSAGE = "Remove the unused parameter \"%s\".";
  ...
  private boolean anyNextSiblingUsed(final AstNode identifierNode) {
    ...
    while (sibling != null) {
      ...
      if (siblingEntry != null
          && sibling.getTokenValue().equals(identifierNode.getTokenValue())
  ...
  protected void walkPostMagik(final AstNode node) {
    ...
    for (final ScopeEntry entry : scopeEntries) {
      ...
      if (entry.isType(ScopeEntry.Type.PARAMETER)) {
